(function () {
  ...
  var absences = [];
  var settingsCard = window.SalonSettingsCard.create();
  ...
  function renderContent(reservations, clients) {
    ...
    var visibleAbsences = absences.filter(function (item) {
      return window.SalonDomain.isCurrentOrUpcomingPeriod(item);
    });
    var myAbsences = sortByStartDate(visibleAbsences.filter(function (item) {
      return item.collab === user.name;
    }));
    var teamAbsences = auth.isAdmin(user)
      ? sortByStartDate(visibleAbsences.filter(function (item) { return item.collab !== user.name; }))
      : [];

    ui.setMain([
      '<div class="cards">',
      window.SalonAppearanceCard.render(user),
      settingsCard.render(),
    ...
    bindActions();
    window.SalonAppearanceCard.bind(user);
